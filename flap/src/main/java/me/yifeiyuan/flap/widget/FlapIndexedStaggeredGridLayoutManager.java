...
public class FlapIndexedStaggeredGridLayoutManager extends RecyclerView.LayoutManager implements
    ...
    private static final String TAG = "FlapIndexedSGLM";
    ...
    FlapIndexedStaggeredGridLayoutManager.Span[] mSpans;
    ...
    @NonNull
    private final IndexedLayoutState mLayoutState;
     ...
     * layout.
     */
    private FlapIndexedStaggeredGridLayoutManager.SavedState mPendingSavedState;
     ...
     * Re-used anchor info.
     */
    private final FlapIndexedStaggeredGridLayoutManager.AnchorInfo mAnchorInfo = new FlapIndexedStaggeredGridLayoutManager.AnchorInfo();
     ...
     */
    @SuppressWarnings("unused")
    public FlapIndexedStaggeredGridLayoutManager(Context context, AttributeSet attrs, int defStyleAttr,
                                                 int defStyleRes) {
     ...
     */
    public FlapIndexedStaggeredGridLayoutManager(int spanCount, int orientation) {
    ...
    boolean checkForGaps() {
        ...
        int invalidGapDir = mShouldReverseLayout ? IndexedLayoutState.LAYOUT_START : IndexedLayoutState.LAYOUT_END;
        final FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem invalidFsi = mLazySpanLookup
        ...
        final FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem validFsi = mLazySpanLookup
    ...
    View hasGapsToFix() {
        ...
        for (int i = firstChildIndex; i != childLimit; i += nextChildDiff) {
            ...
            FlapIndexedStaggeredGridLayoutManager.LayoutParams lp = (FlapIndexedStaggeredGridLayoutManager.LayoutParams) child.getLayoutParams();
            ...
            if (i + nextChildDiff != childLimit) {
                ...
                if (compareSpans) {
                    ...
                    FlapIndexedStaggeredGridLayoutManager.LayoutParams nextLp = (FlapIndexedStaggeredGridLayoutManager.LayoutParams) nextChild.getLayoutParams();
    ...
    private boolean checkSpanForGap(FlapIndexedStaggeredGridLayoutManager.Span span) {
        if (mShouldReverseLayout) {
            if (span.getEndLine() < mPrimaryOrientation.getEndAfterPadding()) {
                ...
                final FlapIndexedStaggeredGridLayoutManager.LayoutParams lp = span.getLayoutParams(endView);
                return !lp.mFullSpan;
            }
        } else if (span.getStartLine() > mPrimaryOrientation.getStartAfterPadding()) {
            // if it is full span, it is OK
            final View startView = span.mViews.get(0);
            final FlapIndexedStaggeredGridLayoutManager.LayoutParams lp = span.getLayoutParams(startView);
            return !lp.mFullSpan;
        }
        return false;
    }
    ...
    public void setSpanCount(int spanCount) {
        ...
        if (spanCount != mSpanCount) {
            ...
            mSpans = new FlapIndexedStaggeredGridLayoutManager.Span[mSpanCount];
            for (int i = 0; i < mSpanCount; i++) {
                mSpans[i] = new FlapIndexedStaggeredGridLayoutManager.Span(i);
            }
            requestLayout();
        }
    }
    ...
    /**
     * Returns the current gap handling strategy for StaggeredGridLayoutManagerT.
     * <p>
    ...
    /**
     * Sets the gap handling strategy for StaggeredGridLayoutManagerT. If the gapStrategy parameter
    ...
    /**
     * Returns the number of spans laid out by StaggeredGridLayoutManagerT.
     *
    ...
    private void onLayoutChildren(RecyclerView.Recycler recycler, RecyclerView.State state,
        ...
        final FlapIndexedStaggeredGridLayoutManager.AnchorInfo anchorInfo = mAnchorInfo;
        ...
        if (getChildCount() > 0 && (mPendingSavedState == null
            ...
            if (anchorInfo.mInvalidateOffsets) {
                ...
                if (recalculateAnchor || mAnchorInfo.mSpanReferenceLines == null) {
                    ...
                    for (int i = 0; i < mSpanCount; i++) {
                        final FlapIndexedStaggeredGridLayoutManager.Span span = mSpans[i];
                        span.clear();
        ...
        if (anchorInfo.mLayoutFromEnd) {
            // Layout start.
            setLayoutStateDirection(IndexedLayoutState.LAYOUT_START);
            fill(recycler, mLayoutState, state);
            // Layout end.
            setLayoutStateDirection(IndexedLayoutState.LAYOUT_END);
    ...
    private void repositionToWrapContentIfNecessary() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            FlapIndexedStaggeredGridLayoutManager.LayoutParams layoutParams = (FlapIndexedStaggeredGridLayoutManager.LayoutParams) child.getLayoutParams();
        ...
        for (int i = 0; i < childCount; i++) {
            View child = getChildAt(i);
            final FlapIndexedStaggeredGridLayoutManager.LayoutParams lp = (FlapIndexedStaggeredGridLayoutManager.LayoutParams) child.getLayoutParams();
            if (lp.mFullSpan) {
                continue;
            }
    ...
    private void applyPendingSavedState(FlapIndexedStaggeredGridLayoutManager.AnchorInfo anchorInfo) {
        if (DEBUG) {
        ...
        if (mPendingSavedState.mSpanOffsetsSize > 0) {
            if (mPendingSavedState.mSpanOffsetsSize == mSpanCount) {
                for (int i = 0; i < mSpanCount; i++) {
                    ...
                    if (line != FlapIndexedStaggeredGridLayoutManager.Span.INVALID_LINE) {
    ...
    void updateAnchorInfoForLayout(RecyclerView.State state, FlapIndexedStaggeredGridLayoutManager.AnchorInfo anchorInfo) {
    ...
    private boolean updateAnchorFromChildren(RecyclerView.State state, FlapIndexedStaggeredGridLayoutManager.AnchorInfo anchorInfo) {
    ...
    boolean updateAnchorFromPendingData(RecyclerView.State state, FlapIndexedStaggeredGridLayoutManager.AnchorInfo anchorInfo) {
        ...
        if (mPendingSavedState == null || mPendingSavedState.mAnchorPosition == RecyclerView.NO_POSITION
            ...
            if (child != null) {
                ...
                if (mPendingScrollPositionOffset == INVALID_OFFSET) {
                    ...
                    anchorInfo.mLayoutFromEnd = position == IndexedLayoutState.LAYOUT_END;
                    anchorInfo.assignCoordinateFromPadding();
                } else {
...
//        return mPendingSavedState == null;
    ...
    private int computeScrollOffset(RecyclerView.State state) {
        ...
        return IndexedScrollbarHelper.computeScrollOffset(state, mPrimaryOrientation,
    ...
    private int computeScrollExtent(RecyclerView.State state) {
        ...
        return IndexedScrollbarHelper.computeScrollExtent(state, mPrimaryOrientation,
                findFirstVisibleItemClosestToStart(!mSmoothScrollbarEnabled),
                findFirstVisibleItemClosestToEnd(!mSmoothScrollbarEnabled),
    ...
    private int computeScrollRange(RecyclerView.State state) {
        ...
        return IndexedScrollbarHelper.computeScrollRange(state, mPrimaryOrientation,
                findFirstVisibleItemClosestToStart(!mSmoothScrollbarEnabled),
                findFirstVisibleItemClosestToEnd(!mSmoothScrollbarEnabled),
                this, mSmoothScrollbarEnabled);
    }
    ...
    private void measureChildWithDecorationsAndMargin(View child, FlapIndexedStaggeredGridLayoutManager.LayoutParams lp,
...
//        }
    ...
    public void onRestoreInstanceState(Parcelable state) {
        if (state instanceof FlapIndexedStaggeredGridLayoutManager.SavedState) {
            mPendingSavedState = (FlapIndexedStaggeredGridLayoutManager.SavedState) state;
    ...
    public Parcelable onSaveInstanceState() {
        if (mPendingSavedState != null) {
            return new FlapIndexedStaggeredGridLayoutManager.SavedState(mPendingSavedState);
        }
        FlapIndexedStaggeredGridLayoutManager.SavedState state = new FlapIndexedStaggeredGridLayoutManager.SavedState();
        ...
        if (getChildCount() > 0) {
            ...
            for (int i = 0; i < mSpanCount; i++) {
                ...
                if (mLastLayoutFromEnd) {
                    line = mSpans[i].getEndLine(FlapIndexedStaggeredGridLayoutManager.Span.INVALID_LINE);
                    if (line != FlapIndexedStaggeredGridLayoutManager.Span.INVALID_LINE) {
                        line -= mPrimaryOrientation.getEndAfterPadding();
                    }
                } else {
                    line = mSpans[i].getStartLine(FlapIndexedStaggeredGridLayoutManager.Span.INVALID_LINE);
                    if (line != FlapIndexedStaggeredGridLayoutManager.Span.INVALID_LINE) {
                        line -= mPrimaryOrientation.getStartAfterPadding();
                    }
                }
    ...
    private void setLayoutStateDirection(int direction) {
        ...
        mLayoutState.mItemDirection = (mShouldReverseLayout == (direction == IndexedLayoutState.LAYOUT_START))
                ? IndexedLayoutState.ITEM_DIRECTION_TAIL : IndexedLayoutState.ITEM_DIRECTION_HEAD;
    }
    ...
    public void onItemsRemoved(RecyclerView recyclerView, int positionStart, int itemCount) {
        handleUpdate(positionStart, itemCount, IndexedAdapterHelper.UpdateOp.REMOVE);
    }

    @Override
    public void onItemsAdded(RecyclerView recyclerView, int positionStart, int itemCount) {
        handleUpdate(positionStart, itemCount, IndexedAdapterHelper.UpdateOp.ADD);
    }
    ...
    public void onItemsMoved(RecyclerView recyclerView, int from, int to, int itemCount) {
        handleUpdate(from, to, IndexedAdapterHelper.UpdateOp.MOVE);
    }

    @Override
    public void onItemsUpdated(RecyclerView recyclerView, int positionStart, int itemCount,
                               Object payload) {
        handleUpdate(positionStart, itemCount, IndexedAdapterHelper.UpdateOp.UPDATE);
    }
    ...
    private void handleUpdate(int positionStart, int itemCountOrToPosition, int cmd) {
        ...
        if (cmd == IndexedAdapterHelper.UpdateOp.MOVE) {
    ...
    private int fill(RecyclerView.Recycler recycler, IndexedLayoutState layoutState,
        ...
        if (mLayoutState.mInfinite) {
            if (layoutState.mLayoutDirection == IndexedLayoutState.LAYOUT_END) {
        ...
        while (layoutState.hasMore(state)
            ...
            FlapIndexedStaggeredGridLayoutManager.LayoutParams lp = ((FlapIndexedStaggeredGridLayoutManager.LayoutParams) view.getLayoutParams());
            ...
            if (layoutState.mLayoutDirection == IndexedLayoutState.LAYOUT_END) {
                ...
                if (assignSpan && lp.mFullSpan) {
                    FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem fullSpanItem;
                    fullSpanItem = createFullSpanItemFromEnd(start);
                    fullSpanItem.mGapDir = IndexedLayoutState.LAYOUT_START;
                ...
                if (assignSpan && lp.mFullSpan) {
                    ...
                    fullSpanItem.mGapDir = IndexedLayoutState.LAYOUT_END;
                    fullSpanItem.mPosition = position;
                    mLazySpanLookup.addFullSpanItem(fullSpanItem);
                }
            }
            ...
            if (lp.mFullSpan && layoutState.mItemDirection == IndexedLayoutState.ITEM_DIRECTION_HEAD) {
                if (assignSpan) {
                    ...
                    if (hasInvalidGap) {
                        final FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem fullSpanItem = mLazySpanLookup
        ...
        final int diff;
        if (mLayoutState.mLayoutDirection == IndexedLayoutState.LAYOUT_START) {
    ...
    private FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem createFullSpanItemFromEnd(int newItemTop) {
        FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem fsi = new FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem();
    ...
    private FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem createFullSpanItemFromStart(int newItemBottom) {
        FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem fsi = new FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem();
        fsi.mGapPerSpan = new int[mSpanCount];
        for (int i = 0; i < mSpanCount; i++) {
    ...
    private void attachViewToSpans(View view, FlapIndexedStaggeredGridLayoutManager.LayoutParams lp, IndexedLayoutState layoutState) {
        if (layoutState.mLayoutDirection == IndexedLayoutState.LAYOUT_END) {
            if (lp.mFullSpan) {
    ...
    private void recycle(RecyclerView.Recycler recycler, IndexedLayoutState layoutState) {
        if (!layoutState.mRecycle || layoutState.mInfinite) {
            return;
        }
        if (layoutState.mAvailable == 0) {
            // easy, recycle line is still valid
            if (layoutState.mLayoutDirection == IndexedLayoutState.LAYOUT_START) {
    ...
    private void updateRemainingSpans(FlapIndexedStaggeredGridLayoutManager.Span span, int layoutDir, int targetLine) {
        final int deletedSize = span.getDeletedSize();
        if (layoutDir == IndexedLayoutState.LAYOUT_START) {
    ...
    boolean areAllEndsEqual() {
        int end = mSpans[0].getEndLine(FlapIndexedStaggeredGridLayoutManager.Span.INVALID_LINE);
        for (int i = 1; i < mSpanCount; i++) {
            if (mSpans[i].getEndLine(FlapIndexedStaggeredGridLayoutManager.Span.INVALID_LINE) != end) {
                return false;
            }
        }
        return true;
    ...
    boolean areAllStartsEqual() {
        int start = mSpans[0].getStartLine(FlapIndexedStaggeredGridLayoutManager.Span.INVALID_LINE);
        for (int i = 1; i < mSpanCount; i++) {
            if (mSpans[i].getStartLine(FlapIndexedStaggeredGridLayoutManager.Span.INVALID_LINE) != start) {
                return false;
            }
        }
        return true;
    ...
    private boolean preferLastSpan(int layoutDir) {
        if (mOrientation == HORIZONTAL) {
            return (layoutDir == IndexedLayoutState.LAYOUT_START) != mShouldReverseLayout;
        }
        return ((layoutDir == IndexedLayoutState.LAYOUT_START) == mShouldReverseLayout) == isLayoutRTL();
    }
    ...
    private FlapIndexedStaggeredGridLayoutManager.Span getNextSpan(IndexedLayoutState layoutState, int viewAdapterPosition) {
        ...
        if (layoutState.mLayoutDirection == IndexedLayoutState.LAYOUT_END) {
            FlapIndexedStaggeredGridLayoutManager.Span min = null;
            int minLine = Integer.MAX_VALUE;
            final int defaultLine = mPrimaryOrientation.getStartAfterPadding();
            for (int i = startIndex; i != endIndex; i += diff) {
                final FlapIndexedStaggeredGridLayoutManager.Span other = mSpans[i];
                int otherLine = other.getEndLine(defaultLine);
                if (otherLine < minLine) {
                    min = other;
                    minLine = otherLine;
                }
                min = mSpans[viewAdapterPosition % getSpanCount()];
            }
            return min;
        } else {
            FlapIndexedStaggeredGridLayoutManager.Span max = null;
    ...
    private int calculateScrollDirectionForPosition(int position) {
        if (getChildCount() == 0) {
            return mShouldReverseLayout ? IndexedLayoutState.LAYOUT_END : IndexedLayoutState.LAYOUT_START;
        }
        final int firstChildPos = getFirstChildPosition();
        return position < firstChildPos != mShouldReverseLayout ? IndexedLayoutState.LAYOUT_START : IndexedLayoutState.LAYOUT_END;
    }
    ...
    public void collectAdjacentPrefetchPositions(int dx, int dy, RecyclerView.State state,
        ...
        for (int i = 0; i < mSpanCount; i++) {
            ...
            int distance = mLayoutState.mItemDirection == IndexedLayoutState.LAYOUT_START
    ...
    void prepareLayoutStateForDelta(int delta, RecyclerView.State state) {
        ...
        if (delta > 0) { // layout towards end
            layoutDir = IndexedLayoutState.LAYOUT_END;
            referenceChildPosition = getLastChildPosition();
        } else {
            layoutDir = IndexedLayoutState.LAYOUT_START;
    ...
    public RecyclerView.LayoutParams generateDefaultLayoutParams() {
        if (mOrientation == HORIZONTAL) {
            return new FlapIndexedStaggeredGridLayoutManager.LayoutParams(ViewGroup.LayoutParams.WRAP_CONTENT,
                    ViewGroup.LayoutParams.MATCH_PARENT);
        } else {
            return new FlapIndexedStaggeredGridLayoutManager.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT,
    ...
    public RecyclerView.LayoutParams generateLayoutParams(Context c, AttributeSet attrs) {
        return new FlapIndexedStaggeredGridLayoutManager.LayoutParams(c, attrs);
    }

    @Override
    public RecyclerView.LayoutParams generateLayoutParams(ViewGroup.LayoutParams lp) {
        if (lp instanceof ViewGroup.MarginLayoutParams) {
            return new FlapIndexedStaggeredGridLayoutManager.LayoutParams((ViewGroup.MarginLayoutParams) lp);
        } else {
            return new FlapIndexedStaggeredGridLayoutManager.LayoutParams(lp);
        }
    }

    @Override
    public boolean checkLayoutParams(RecyclerView.LayoutParams lp) {
        return lp instanceof FlapIndexedStaggeredGridLayoutManager.LayoutParams;
    }
    ...
    public View onFocusSearchFailed(View focused, int direction, RecyclerView.Recycler recycler,
        ...
        if (layoutDir == IndexedLayoutState.INVALID_LAYOUT) {
            return null;
        }
        FlapIndexedStaggeredGridLayoutManager.LayoutParams prevFocusLayoutParams = (FlapIndexedStaggeredGridLayoutManager.LayoutParams) directChild.getLayoutParams();
        boolean prevFocusFullSpan = prevFocusLayoutParams.mFullSpan;
        final FlapIndexedStaggeredGridLayoutManager.Span prevFocusSpan = prevFocusLayoutParams.mSpan;
        final int referenceChildPosition;
        if (layoutDir == IndexedLayoutState.LAYOUT_END) { // layout towards end
            referenceChildPosition = getLastChildPosition();
        } else {
            referenceChildPosition = getFirstChildPosition();
        }
        ...
        boolean shouldSearchFromStart = !mReverseLayout == (layoutDir == IndexedLayoutState.LAYOUT_START);
    ...
    private int convertFocusDirectionToLayoutDirection(int focusDirection) {
        switch (focusDirection) {
            case View.FOCUS_BACKWARD:
                if (mOrientation == VERTICAL) {
                    return IndexedLayoutState.LAYOUT_START;
                } else if (isLayoutRTL()) {
                    return IndexedLayoutState.LAYOUT_END;
                } else {
                    return IndexedLayoutState.LAYOUT_START;
                }
            ...
            case View.FOCUS_UP:
                return mOrientation == VERTICAL ? IndexedLayoutState.LAYOUT_START
                        : IndexedLayoutState.INVALID_LAYOUT;
            case View.FOCUS_DOWN:
                return mOrientation == VERTICAL ? IndexedLayoutState.LAYOUT_END
                        : IndexedLayoutState.INVALID_LAYOUT;
            case View.FOCUS_LEFT:
                return mOrientation == HORIZONTAL ? IndexedLayoutState.LAYOUT_START
                        : IndexedLayoutState.INVALID_LAYOUT;
            case View.FOCUS_RIGHT:
                return mOrientation == HORIZONTAL ? IndexedLayoutState.LAYOUT_END
                        : IndexedLayoutState.INVALID_LAYOUT;
            default:
                if (DEBUG) {
                    Log.d(TAG, "Unknown focus request:" + focusDirection);
                }
                return IndexedLayoutState.INVALID_LAYOUT;
        }

    }

    /**
     * LayoutParams used by StaggeredGridLayoutManagerT.
     * <p>
    ...
    public static class LayoutParams extends RecyclerView.LayoutParams {
        ...
        FlapIndexedStaggeredGridLayoutManager.Span mSpan;
    ...
    class Span {
        ...
        void calculateCachedStart() {
            ...
            if (lp.mFullSpan) {
                FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem fsi = mLazySpanLookup
                        .getFullSpanItem(lp.getViewLayoutPosition());
                if (fsi != null && fsi.mGapDir == IndexedLayoutState.LAYOUT_START) {
        ...
        void calculateCachedEnd() {
            ...
            final FlapIndexedStaggeredGridLayoutManager.LayoutParams lp = getLayoutParams(endView);
            mCachedEnd = mPrimaryOrientation.getDecoratedEnd(endView);
            if (lp.mFullSpan) {
                FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem fsi = mLazySpanLookup
                        .getFullSpanItem(lp.getViewLayoutPosition());
                if (fsi != null && fsi.mGapDir == IndexedLayoutState.LAYOUT_END) {
        ...
        void prependToSpan(View view) {
            FlapIndexedStaggeredGridLayoutManager.LayoutParams lp = getLayoutParams(view);
            lp.mSpan = this;
            mViews.add(0, view);
        ...
        void popEnd() {
            ...
            final FlapIndexedStaggeredGridLayoutManager.LayoutParams lp = getLayoutParams(end);
            lp.mSpan = null;
            if (lp.isItemRemoved() || lp.isItemChanged()) {
        ...
        void popStart() {
            ...
            final FlapIndexedStaggeredGridLayoutManager.LayoutParams lp = getLayoutParams(start);
            lp.mSpan = null;
            if (mViews.size() == 0) {
                mCachedEnd = INVALID_LINE;
            }
        ...
        FlapIndexedStaggeredGridLayoutManager.LayoutParams getLayoutParams(View view) {
            return (FlapIndexedStaggeredGridLayoutManager.LayoutParams) view.getLayoutParams();
        }
    ...
    static class LazySpanLookup {
        ...
        int forceInvalidateAfter(int position) {
            if (mFullSpanItems != null) {
                for (int i = mFullSpanItems.size() - 1; i >= 0; i--) {
                    FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem fsi = mFullSpanItems.get(i);
        ...
        int invalidateAfter(int position) {
            ...
            if (endPosition == RecyclerView.NO_POSITION) {
                Arrays.fill(mData, position, mData.length, FlapIndexedStaggeredGridLayoutManager.LayoutParams.INVALID_SPAN_ID);
                return mData.length;
            } else {
                ...
                Arrays.fill(mData, position, invalidateToIndex, FlapIndexedStaggeredGridLayoutManager.LayoutParams.INVALID_SPAN_ID);
        ...
        int getSpan(int position) {
            if (mData == null || position >= mData.length) {
                return FlapIndexedStaggeredGridLayoutManager.LayoutParams.INVALID_SPAN_ID;
            } else {
                return mData[position];
            }
        }

        void setSpan(int position, FlapIndexedStaggeredGridLayoutManager.Span span) {
            ensureSize(position);
        ...
        void ensureSize(int position) {
            if (mData == null) {
                ...
                Arrays.fill(mData, FlapIndexedStaggeredGridLayoutManager.LayoutParams.INVALID_SPAN_ID);
            } else if (position >= mData.length) {
                int[] old = mData;
                mData = new int[sizeForPosition(position)];
                System.arraycopy(old, 0, mData, 0, old.length);
                Arrays.fill(mData, old.length, mData.length, FlapIndexedStaggeredGridLayoutManager.LayoutParams.INVALID_SPAN_ID);
            }
        }
        ...
        void offsetForRemoval(int positionStart, int itemCount) {
            ...
            Arrays.fill(mData, mData.length - itemCount, mData.length,
                    FlapIndexedStaggeredGridLayoutManager.LayoutParams.INVALID_SPAN_ID);
        ...
        private int invalidateFullSpansAfter(int position) {
            ...
            final FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem item = getFullSpanItem(position);
            ...
            if (nextFsiIndex != -1) {
                FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem fsi = mFullSpanItems.get(nextFsiIndex);
        ...
        public void addFullSpanItem(FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem fullSpanItem) {
            if (mFullSpanItems == null) {
                mFullSpanItems = new ArrayList<>();
            }
            final int size = mFullSpanItems.size();
            for (int i = 0; i < size; i++) {
                FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem other = mFullSpanItems.get(i);
        ...
        public FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem getFullSpanItem(int position) {
            if (mFullSpanItems == null) {
                return null;
            }
            for (int i = mFullSpanItems.size() - 1; i >= 0; i--) {
                final FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem fsi = mFullSpanItems.get(i);
                if (fsi.mPosition == position) {
                    return fsi;
                }
            }
        ...
        static class FullSpanItem implements Parcelable {
            ...
            public static final Parcelable.Creator<FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem> CREATOR =
                    new Parcelable.Creator<FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem>() {
                        @Override
                        public FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem createFromParcel(Parcel in) {
                            return new FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem(in);
                        }

                        @Override
                        public FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem[] newArray(int size) {
                            return new FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem[size];
                        }
                    };
        }
    }
    ...
    public static class SavedState implements Parcelable {
        ...
        int mSpanLookupSize;
        int[] mSpanLookup;
        List<FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem> mFullSpanItems;
        ...
        SavedState(Parcel in) {
            ...
            List<FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem> fullSpanItems =
                    in.readArrayList(FlapIndexedStaggeredGridLayoutManager.LazySpanLookup.FullSpanItem.class.getClassLoader());
            mFullSpanItems = fullSpanItems;
        }

        public SavedState(FlapIndexedStaggeredGridLayoutManager.SavedState other) {
        ...
        public static final Parcelable.Creator<FlapIndexedStaggeredGridLayoutManager.SavedState> CREATOR =
                new Parcelable.Creator<FlapIndexedStaggeredGridLayoutManager.SavedState>() {
                    @Override
                    public FlapIndexedStaggeredGridLayoutManager.SavedState createFromParcel(Parcel in) {
                        return new FlapIndexedStaggeredGridLayoutManager.SavedState(in);
                    }

                    @Override
                    public FlapIndexedStaggeredGridLayoutManager.SavedState[] newArray(int size) {
                        return new FlapIndexedStaggeredGridLayoutManager.SavedState[size];
                    }
                };
    }
    ...
    class AnchorInfo {
        ...
        void saveSpanReferenceLines(FlapIndexedStaggeredGridLayoutManager.Span[] spans) {
            ...
            for (int i = 0; i < spanCount; i++) {
                ...
                mSpanReferenceLines[i] = spans[i].getStartLine(FlapIndexedStaggeredGridLayoutManager.Span.INVALID_LINE);
            }
        }
